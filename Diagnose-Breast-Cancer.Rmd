...
The barplot below demonstrates that a significant amount of patients are white while very few of them are Native American or mixed race.
```{r}
par(mar=c(5, 10, 4, 2))
...
Out of 30 percent of the sample data, we can see that majority of the partients do not have breast cancer. Out of those that do, they seem to fall for those who were thirty years old when they gave birth to their first child.
```{r}
par(mar=c(5, 10, 4, 2))
...
Here we can see that this new model predicted recall more than ninety percent of the time.
...
The data from the double density plot below illustrates how no breast cancer was predicted most of the time meaning 
...
Errors on both training data and testing data are calculated. 
```{r}
te_errs = c()
tr_errs = c()
...
```{r}
plot(tr_sizes, tr_errs, col="firebrick", type="b", ylim=range(c(te_errs,tr_errs)), ylab="Classification Error", xlab="Training Set Size", main="Learning Curve for features Age Group, Breast Density, Relatives With Breast Cancer, Invasive Breast Cancer, and Menopause")
...
The new model predicted recall more than ninety percent of the time.
...
Here we can also see how heart disease was predicted most of the time. 
...
The Naive Bayes model will include the features age group, menopause, breast density, relatives with breast cancer, and invasive breast cancer. All which are the features from our hypothesis. By printing the mean of the 
```{r}
...
Here we can see that the first model based on the precison we can see that all our prediction with cancer were correct. As for our recall we can see that the model predicted both items correctly seventy five percent of the time.
```{r}
# precision = true positives / (true positives + false positives)
precision = TP/(TP+FP)
...
From this new confusion Matrix we can see that the classier gets segnificantly worse.
```{r}
actual = te_dat$cancer_factor
predicted = predict(fit, newdata = te_dat)
...
The overall recall for this model dropped from .75 to .69. Ideally the recall should be as close as possible to one.
```{r}
# precision = true positives / (true positives + false positives)
precision = TP/(TP+FP)
