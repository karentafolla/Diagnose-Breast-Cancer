...
```{r}
plot(tr_sizes, tr_errs, col="firebrick", type="b", ylim=range(c(te_errs,tr_errs)), ylab="Classification Error", xlab="Training Set Size", main="Learning Curve for features maxhr, thal, dep")
...
We can see that the accuracy dropped by .2 percent. 
```{r}
accuracy = mean(predicted == actual)
round(accuracy, 3)
```
...
From this confusion matrix we can see that the density of the error is almost at one percent.
```{r}
plot(density(as.numeric(predicted[actual == "cancer"])), main = "Double Density Plot", xlab="Decision Tree Output", col="red", ylim=range(0:3))
lines(density(as.numeric(predicted[actual == "no cancer"])), col="blue")
legend(1.3,2, c("Cancer", "No Cancer"),col=c("red", "blue"), inset=0.05, lty=1, lwd=3)
...
From this new confusion Matrix we can see that the the results are the same as the previous model.
```{r}
actual = te_dat$cancer_factor
predicted = predict(fit, newdata = te_dat)
