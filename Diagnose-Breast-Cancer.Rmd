...
```{r}
fit1 = naiveBayes(cancer_factor ~ age.group + menopause + breast.density + relatives.with.bc + invasive.bc, data = tr_dat)

class(fit1)
summary(fit1)
print(fit1$tables$menopause)
print(fit1$tables$relatives.with.bc)
...
Although the Naive Bayes classification does not guarantee that an incremented amount of features will improve the classifier. We will test out the classifier with all features provided in order to see whether or not we chose the right features for the model to use. 
```{r}
fit2 = naiveBayes(cancer_factor ~ ., data = tr_dat)
...
From this new confusion Matrix we can see that the 
```{r}
actual = te_dat$cancer_factor
...
```{r}
fit1 = naiveBayes(cancer_factor ~ age.group + last.mammogram + age.first.birth, data = tr_dat)
...
```{r}
fit2 = rpart(cancer_factor ~ age.group + breast.density + relatives.with.bc + invasive.bc, data = tr_dat, method="class")
...
provided an almost 100 percent accuracy for our predictions. 
```{r}
